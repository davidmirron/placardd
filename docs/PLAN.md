...
> **Launch decision (Sep 2026): fixed price only.** Auctions are fully built (bid rules, anti-sniping, settlement) but switched off behind `NEXT_PUBLIC_AUCTIONS_ENABLED` and are not part of the product. Reason: a bid is a promise, not a payment — with no card on file a brand can win every auction and never pay. Every spot has one price, "Buy now" goes straight to Stripe/test checkout, and the spot stays on sale until payment clears (clicking Buy now does not hold it). The "bidding deadline" is an optional "available until" date. The sections below describe the full design including auctions; anything about bidding applies only when the flag is on.
>
